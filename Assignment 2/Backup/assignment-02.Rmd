...
Generate a scatter plot of `bf` on the vertical axis versus `obesity` on the horizontal axis.
...
print(obesity)
print(bp)
plot(obesity, bp,ylab="BP", xlab="OBESITY")        
```
...
cor(bp.obese$obese, bp.obese$bp)

#Covariance
cov(bp.obese$obese, bp.obese$bp)
...
r <- sum(subset(three.dice$RowSUM,three.dice$RowSUM > 12 & three.dice$RowSUM < 18 ))
r
```
...
```{r}
#Insert your answer here
f <- sum(subset(three.dice$RowSUM,three.dice$RowSUM %% 2 == 0))
f
```
...
```{r}
#Insert your answer here
M <- mean(subset(three.dice$RowSUM,three.dice$RowMeans == 4))
M
```
******
\newpage
